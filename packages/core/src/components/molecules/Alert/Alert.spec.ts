...
test.describe('Alert', () => {
  test('renders correctly', async ({ page }) => {
    await gotoStory(page, 'core-alert--default');

    const component = page.locator('[class*="alert"]').first();
    await expect(component).toBeVisible();
  });

  test('displays all size variants', async ({ page }) => {
    await gotoStory(page, 'core-alert--sizes');

    const components = page.locator('[class*="alert"]');
    await expect(components).toHaveCount(6); // xs, sm, md, lg, xl, 2xl
  });

  test('displays all variants', async ({ page }) => {
    await gotoStory(page, 'core-alert--variants');
