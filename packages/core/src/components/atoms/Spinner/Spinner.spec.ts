...
test.describe('Spinner', () => {
  test('renders correctly', async ({ page }) => {
    await gotoStory(page, 'core-spinner--default');

    const component = page.locator('[class*="spinner"]').first();
    await expect(component).toBeVisible();
  });

  test('displays all size variants', async ({ page }) => {
    await gotoStory(page, 'core-spinner--sizes');

    const components = page.locator('[class*="spinner"]');
    await expect(components).toHaveCount(6); // xs, sm, md, lg, xl, 2xl
  });

  test('displays all variants', async ({ page }) => {
    await gotoStory(page, 'core-spinner--variants');
