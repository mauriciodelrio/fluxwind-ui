...
test.describe('Text', () => {
  test('renders correctly', async ({ page }) => {
    await gotoStory(page, 'core-text--default');

    const component = page.locator('[class*="text"]').first();
    await expect(component).toBeVisible();
  });

  test('displays all size variants', async ({ page }) => {
    await gotoStory(page, 'core-text--sizes');

    const components = page.locator('[class*="text"]');
    await expect(components).toHaveCount(6); // xs, sm, md, lg, xl, 2xl
  });

  test('displays all variants', async ({ page }) => {
    await gotoStory(page, 'core-text--variants');
